...
- **Granular Synthesis Engine**: Real-time AudioWorklet-based granular synthesis with sample-accurate grain scheduling. Features two modes (Cinematic and Design), sample input (capture synth or drag-drop), density-based grain scheduling, various window types (Hann, Gauss, Blackman, Rect), and post-bus processing including anti-mud rules. Parameters update in real-time during playback via a main-thread scheduler that sends grain events ahead to the AudioWorkletProcessor. Includes an independent AHD envelope for envelope control.
