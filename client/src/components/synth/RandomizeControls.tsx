...
export function RandomizeControls({ currentParams, onRandomize }: RandomizeControlsProps) {
  ...
  const randomizeAll = () => {
    ...
    const params: SynthParameters = {
      ...
      mastering: {
        compressorEnabled: Math.random() > 0.3,
        compressorThreshold: Math.round(randomInRange(-20, -6)),
        compressorRatio: Math.round(randomInRange(2, 8)),
        compressorAttack: Math.round(randomInRange(1, 30) * 10) / 10,
        compressorRelease: Math.round(randomInRange(50, 300)),
        compressorKnee: Math.round(randomInRange(0, 20)),
        compressorMakeup: Math.round(randomInRange(0, 12)),
        exciterEnabled: Math.random() > 0.4,
        exciterFreq: Math.round(randomInRange(4000, 10000, true)),
        exciterAmount: Math.round(randomInRange(10, 60 * chaos)),
        exciterMix: Math.round(randomInRange(30, 70)),
        stereoEnabled: Math.random() > 0.7,
        stereoWidth: Math.round(randomInRange(80, 150)),
      },
      output: {
        volume: 75,
  ...
  const mutate = () => {
    ...
    const params: SynthParameters = {
      ...
      mastering: {
        compressorEnabled: currentParams.mastering.compressorEnabled,
        compressorThreshold: Math.round(mutateValue(currentParams.mastering.compressorThreshold, -40, 0)),
        compressorRatio: Math.round(mutateValue(currentParams.mastering.compressorRatio, 1, 20) * 10) / 10,
        compressorAttack: Math.round(mutateValue(currentParams.mastering.compressorAttack, 0.1, 100) * 10) / 10,
        compressorRelease: Math.round(mutateValue(currentParams.mastering.compressorRelease, 10, 1000)),
        compressorKnee: Math.round(mutateValue(currentParams.mastering.compressorKnee, 0, 40)),
        compressorMakeup: Math.round(mutateValue(currentParams.mastering.compressorMakeup, 0, 24) * 10) / 10,
        exciterEnabled: currentParams.mastering.exciterEnabled,
        exciterFreq: Math.round(mutateValue(currentParams.mastering.exciterFreq, 2000, 12000, true)),
        exciterAmount: Math.round(mutateValue(currentParams.mastering.exciterAmount, 0, 100)),
        exciterMix: Math.round(mutateValue(currentParams.mastering.exciterMix, 0, 100)),
        stereoEnabled: currentParams.mastering.stereoEnabled,
        stereoWidth: Math.round(mutateValue(currentParams.mastering.stereoWidth, 0, 200)),
      },
      output: {
